    ...
    : SecureFlightBaseController(mapper)
{
    ...
    [HttpPost]
    [ProducesResponseType(typeof(AirportDataTransferObject), StatusCodes.Status201Created)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(ErrorResponseActionResult))]
    public async Task<IActionResult> Post(AirportDataTransferObject airportDto)
    {
        var airport = mapper.Map<AirportDataTransferObject, Airport>(airportDto);
        var result = airportRepository.New(airport);
        await airportRepository.SaveChangesAsync();
        return MapResultToDataTransferObject<Airport, AirportDataTransferObject>(result);
    }

    [HttpDelete("{code}")]
    [ProducesResponseType(typeof(AirportDataTransferObject), StatusCodes.Status202Accepted)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(ErrorResponseActionResult))]
    public async Task<IActionResult> Delete([FromRoute] string code, AirportDataTransferObject airportDto)
    {
        var airport = await airportRepository.GetByIdAsync(code);
        if (airport is null)
        {
        ...
        airport.Country = airportDto.Country;
        airport.Name = airportDto.Name;
        var result = airportRepository.Delete(airport);
        await airportRepository.SaveChangesAsync();
        return MapResultToDataTransferObject<Airport, AirportDataTransferObject>(result);
    }
}